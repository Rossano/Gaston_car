...
// Include instance config 
 #include "sl_iostream_eusart_stm32_config.h"
...
#if (SL_IOSTREAM_EUSART_CLOCK_SOURCE(SL_IOSTREAM_EUSART_STM32_PERIPHERAL_NO) == \
    SL_IOSTREAM_EUSART_DISABLED_CLOCK_SOURCE)
  #error Peripheral clock is disabled for STM32. Modify sl_clock_manager_tree_config.h \
  to enable the peripheral clock
#else
#define _SL_IOSTREAM_EUSART_STM32_ENABLE_HIGH_FREQUENCY \
          SL_IOSTREAM_EUSART_CLOCK_SOURCE(SL_IOSTREAM_EUSART_STM32_PERIPHERAL_NO) == \
          SL_IOSTREAM_EUSART_HF_CLOCK_SOURCE

#if defined(SL_IOSTREAM_EUSART_STM32_ENABLE_HIGH_FREQUENCY) && \
  (_SL_IOSTREAM_EUSART_STM32_ENABLE_HIGH_FREQUENCY != \
  SL_IOSTREAM_EUSART_STM32_ENABLE_HIGH_FREQUENCY)
#if SL_IOSTREAM_EUSART_STM32_ENABLE_HIGH_FREQUENCY
#warning Configuration mismatch for IOStream EUSART STM32. \
...
#warning Configuration mismatch for IOStream EUSART STM32. \
 ...
 oscillator in sl_clock_manager_tree_config.h.
#endif // SL_IOSTREAM_EUSART_STM32_ENABLE_HIGH_FREQUENCY
...
sl_status_t sl_iostream_eusart_init_stm32(void);

sl_status_t sl_iostream_eusart_init_vcom(void);


// Instance(s) handle and context variable 
/* STM32 EUSART instance disabled to avoid undefined peripheral macros.
   If you want to re-enable it, configure SL_IOSTREAM_EUSART_STM32 in
   config/sl_iostream_eusart_stm32_config.h (use Simplicity Studio Project Configurator)
   and remove the surrounding #if 0/#endif below. */
#if 0
...
static sl_iostream_eusart_context_t  context_stm32;

static uint8_t  rx_buffer_stm32[SL_IOSTREAM_EUSART_STM32_RX_BUFFER_SIZE];

static sli_iostream_uart_periph_t uart_periph_stm32 = {
  .rx_irq_number = SL_IOSTREAM_EUSART_RX_IRQ_NUMBER(SL_IOSTREAM_EUSART_STM32_PERIPHERAL_NO),
#if defined(SL_CATALOG_POWER_MANAGER_PRESENT)  
  .tx_irq_number = SL_IOSTREAM_EUSART_TX_IRQ_NUMBER(SL_IOSTREAM_EUSART_STM32_PERIPHERAL_NO),
#endif
};
...
sl_iostream_instance_info_t sl_iostream_instance_stm32_info = {
  ...
  .periph_id = SL_IOSTREAM_EUSART_STM32_PERIPHERAL_NO,
  .init = sl_iostream_eusart_init_stm32,
};
#endif
...
sl_status_t sl_iostream_eusart_init_stm32(void)
{
  ...
  sl_iostream_eusart_config_t config_stm32 = { 
    .eusart = SL_IOSTREAM_EUSART_PERIPHERAL(SL_IOSTREAM_EUSART_STM32_PERIPHERAL_NO),
    .eusart_nbr = SL_IOSTREAM_EUSART_STM32_PERIPHERAL_NO,
    .bus_clock = SL_IOSTREAM_EUSART_CLOCK_REF(SL_IOSTREAM_EUSART_STM32_PERIPHERAL_NO),
    .baudrate = SL_IOSTREAM_EUSART_STM32_BAUDRATE,
    .parity = SL_IOSTREAM_EUSART_STM32_PARITY,
    .flow_control = SL_IOSTREAM_EUSART_STM32_FLOW_CONTROL_TYPE,
    .stop_bits = SL_IOSTREAM_EUSART_STM32_STOP_BITS,
#if defined(EUSART_COUNT) && (EUSART_COUNT > 1)
    .port_index = SL_IOSTREAM_EUSART_STM32_PERIPHERAL_NO,
#endif
    .tx_port = SL_IOSTREAM_EUSART_STM32_TX_PORT,
    .tx_pin = SL_IOSTREAM_EUSART_STM32_TX_PIN,
    .rx_port = SL_IOSTREAM_EUSART_STM32_RX_PORT,
    .rx_pin = SL_IOSTREAM_EUSART_STM32_RX_PIN,
#if defined(SL_IOSTREAM_EUSART_STM32_CTS_PORT)
    .cts_port = SL_IOSTREAM_EUSART_STM32_CTS_PORT,
    .cts_pin = SL_IOSTREAM_EUSART_STM32_CTS_PIN,
#endif
#if defined(SL_IOSTREAM_EUSART_STM32_RTS_PORT)
    .rts_port = SL_IOSTREAM_EUSART_STM32_RTS_PORT,
    .rts_pin = SL_IOSTREAM_EUSART_STM32_RTS_PIN,
#endif
  };

  sl_iostream_dma_config_t rx_dma_config_stm32 = {.src = (uint8_t *)&SL_IOSTREAM_EUSART_STM32_PERIPHERAL->RXDATA,
                                                        .xfer_cfg = IOSTREAM_LDMA_TFER_CFG_PERIPH(SL_IOSTREAM_EUSART_RX_DMA_SIGNAL(SL_IOSTREAM_EUSART_STM32_PERIPHERAL_NO))};

  sl_iostream_dma_config_t tx_dma_config_stm32 = {.dst = (uint8_t *)&SL_IOSTREAM_EUSART_STM32_PERIPHERAL->TXDATA,
                                                        .xfer_cfg = IOSTREAM_LDMA_TFER_CFG_PERIPH(SL_IOSTREAM_EUSART_TX_DMA_SIGNAL(SL_IOSTREAM_EUSART_STM32_PERIPHERAL_NO))};
  ...
  sl_iostream_uart_config_t uart_config_stm32 = {
    ...
    .rx_buffer_length = SL_IOSTREAM_EUSART_STM32_RX_BUFFER_SIZE,
    .enable_high_frequency = _SL_IOSTREAM_EUSART_STM32_ENABLE_HIGH_FREQUENCY, 
    .lf_to_crlf = SL_IOSTREAM_EUSART_STM32_CONVERT_BY_DEFAULT_LF_TO_CRLF,
    .rx_when_sleeping = SL_IOSTREAM_EUSART_STM32_RESTRICT_ENERGY_MODE_TO_ALLOW_RECEPTION,
    .uart_periph = &uart_periph_stm32
  };
  uart_config_stm32.sw_flow_control = SL_IOSTREAM_EUSART_STM32_FLOW_CONTROL_TYPE == SL_IOSTREAM_EUSART_UART_FLOW_CTRL_SOFT;
#if defined(SL_IOSTREAM_EUSART_STM32_ASYNC_TX)
  uart_config_stm32.async_tx_enabled = SL_IOSTREAM_EUSART_STM32_ASYNC_TX;
#else
  uart_config_stm32.async_tx_enabled = false;
#endif
  // Instantiate eusart instance 
  status = sl_iostream_eusart_init(&sl_iostream_stm32,
                                  &uart_config_stm32,
                                  &config_stm32,
                                  &context_stm32);
  ...
  if (!uart_config_stm32.sw_flow_control) {
    switch (SL_IOSTREAM_EUSART_STM32_FLOW_CONTROL_TYPE)
    {
  ...
  sl_cos_config_vcom((uint32_t) SL_IOSTREAM_EUSART_STM32_BAUDRATE, flow_control);
...
/* STM32 IRQ handlers disabled because STM32 EUSART instance is removed */
#if 0
void SL_IOSTREAM_EUSART_TX_IRQ_HANDLER(SL_IOSTREAM_EUSART_STM32_PERIPHERAL_NO)(void)
{
  sl_iostream_eusart_irq_handler(&sl_iostream_stm32);
}

void SL_IOSTREAM_EUSART_RX_IRQ_HANDLER(SL_IOSTREAM_EUSART_STM32_PERIPHERAL_NO)(void)
{
  sl_iostream_eusart_irq_handler(&sl_iostream_stm32);
}
#endif
