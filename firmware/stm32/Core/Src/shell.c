 ...
 *      Author: rossa
 *      Taken from Fithub: https://github.com/Nanaud7/shell-stm32/tree/main/stm32-freertos
 */
...
#include "stm32f4xx_hal.h"  // for HAL_UARTEx
#include <stdarg.h>
//#include <stdio.h>
...
UART_HandleTypeDef* vcom_huart = NULL;

char starting[] = "\r\n\r\n===== Shell =====\r\n"; // can be modified
char prompt[] = "@STM32 >> "; // can be modified

volatile uint8_t shell_rx_char = 0;
volatile uint8_t vcom_rx_char = 0;
uint8_t shell_rx_buffer[RX_BUFFER_SIZE];
uint8_t vcom_rx_buffer[RX_BUFFER_SIZE];
...
uint8_t __io_putchar(int ch) {
	...
	if(HAL_OK != HAL_UART_Transmit(vcom_huart, (uint8_t *)&ch, 1, HAL_MAX_DELAY)){
		return 1;
	}
	return 0;
}
...
int printf(const char *format, ...) {
	...
	uart_write(buffer, len);  // Use uart_write to VCOM
	va_end(args);
	return len;
}
...
uint8_t uart_write(char *s, uint16_t size) {
	...
	//HAL_UART_Transmit(&huart2, (uint8_t *)s, size, 0xFFFF);
	// Send to VCOM for debug
	printf("%s\n\r", s);
...
uint8_t shell_init(UART_HandleTypeDef* huart) {
	shell_huart = huart;
	vcom_huart = &huart2; // VCOM for debug
	...
	shell_add("help", (void *)sh_help, "Menu d'aide");
	shell_add("f", (void *)sh_example, "Exemple de commande");

	qShell = xQueueCreate(32, sizeof(uint8_t));  // increase queue size

	if (HAL_OK != HAL_UARTEx_ReceiveToIdle_DMA(shell_huart, shell_rx_buffer, RX_BUFFER_SIZE) ||
		HAL_OK != HAL_UARTEx_ReceiveToIdle_DMA(vcom_huart, vcom_rx_buffer, RX_BUFFER_SIZE)) {
		return 1;
	}
	return 0;
}
...
uint8_t shell_char_received() {
	uint8_t ch = 0;
	xQueueReceive(qShell, &ch, portMAX_DELAY);

	switch (ch) {
	case '\n': // Dummy to have double conditions for CR
	case '\r':
		// ENTER key pressed
		printf("\r\n");
		buf[pos++] = 0;
