...
// Include instance config 
 #include "sl_iostream_eusart_mcu_uart_config.h"
...
#if (SL_IOSTREAM_EUSART_CLOCK_SOURCE(SL_IOSTREAM_EUSART_MCU_UART_PERIPHERAL_NO) == \
    SL_IOSTREAM_EUSART_DISABLED_CLOCK_SOURCE)
  #error Peripheral clock is disabled for MCU_UART. Modify sl_clock_manager_tree_config.h \
  to enable the peripheral clock
#else
#define _SL_IOSTREAM_EUSART_MCU_UART_ENABLE_HIGH_FREQUENCY \
          SL_IOSTREAM_EUSART_CLOCK_SOURCE(SL_IOSTREAM_EUSART_MCU_UART_PERIPHERAL_NO) == \
          SL_IOSTREAM_EUSART_HF_CLOCK_SOURCE

#if defined(SL_IOSTREAM_EUSART_MCU_UART_ENABLE_HIGH_FREQUENCY) && \
  (_SL_IOSTREAM_EUSART_MCU_UART_ENABLE_HIGH_FREQUENCY != \
  SL_IOSTREAM_EUSART_MCU_UART_ENABLE_HIGH_FREQUENCY)
#if SL_IOSTREAM_EUSART_MCU_UART_ENABLE_HIGH_FREQUENCY
#warning Configuration mismatch for IOStream EUSART MCU_UART. \
...
#warning Configuration mismatch for IOStream EUSART MCU_UART. \
 ...
 oscillator in sl_clock_manager_tree_config.h.
#endif // SL_IOSTREAM_EUSART_MCU_UART_ENABLE_HIGH_FREQUENCY
...
sl_status_t sl_iostream_eusart_init_mcu_uart(void);

sl_status_t sl_iostream_eusart_init_vcom(void);


// Instance(s) handle and context variable 
static sl_iostream_uart_t sl_iostream_mcu_uart;
sl_iostream_t *sl_iostream_mcu_uart_handle = &sl_iostream_mcu_uart.stream;

sl_iostream_uart_t *sl_iostream_uart_mcu_uart_handle = &sl_iostream_mcu_uart;
static sl_iostream_eusart_context_t  context_mcu_uart;

static uint8_t  rx_buffer_mcu_uart[SL_IOSTREAM_EUSART_MCU_UART_RX_BUFFER_SIZE];

static sli_iostream_uart_periph_t uart_periph_mcu_uart = {
  .rx_irq_number = SL_IOSTREAM_EUSART_RX_IRQ_NUMBER(SL_IOSTREAM_EUSART_MCU_UART_PERIPHERAL_NO),
#if defined(SL_CATALOG_POWER_MANAGER_PRESENT)  
  .tx_irq_number = SL_IOSTREAM_EUSART_TX_IRQ_NUMBER(SL_IOSTREAM_EUSART_MCU_UART_PERIPHERAL_NO),
#endif
};

sl_iostream_instance_info_t sl_iostream_instance_mcu_uart_info = {
  .handle = &sl_iostream_mcu_uart.stream,
  .name = "mcu_uart",
  .type = SL_IOSTREAM_TYPE_UART,
  .periph_id = SL_IOSTREAM_EUSART_MCU_UART_PERIPHERAL_NO,
  .init = sl_iostream_eusart_init_mcu_uart,
};
...
sl_status_t sl_iostream_eusart_init_mcu_uart(void)
{
  ...
  sl_iostream_eusart_config_t config_mcu_uart = { 
    .eusart = SL_IOSTREAM_EUSART_PERIPHERAL(SL_IOSTREAM_EUSART_MCU_UART_PERIPHERAL_NO),
    .eusart_nbr = SL_IOSTREAM_EUSART_MCU_UART_PERIPHERAL_NO,
    .bus_clock = SL_IOSTREAM_EUSART_CLOCK_REF(SL_IOSTREAM_EUSART_MCU_UART_PERIPHERAL_NO),
    .baudrate = SL_IOSTREAM_EUSART_MCU_UART_BAUDRATE,
    .parity = SL_IOSTREAM_EUSART_MCU_UART_PARITY,
    .flow_control = SL_IOSTREAM_EUSART_MCU_UART_FLOW_CONTROL_TYPE,
    .stop_bits = SL_IOSTREAM_EUSART_MCU_UART_STOP_BITS,
#if defined(EUSART_COUNT) && (EUSART_COUNT > 1)
    .port_index = SL_IOSTREAM_EUSART_MCU_UART_PERIPHERAL_NO,
#endif
    .tx_port = SL_IOSTREAM_EUSART_MCU_UART_TX_PORT,
    .tx_pin = SL_IOSTREAM_EUSART_MCU_UART_TX_PIN,
    .rx_port = SL_IOSTREAM_EUSART_MCU_UART_RX_PORT,
    .rx_pin = SL_IOSTREAM_EUSART_MCU_UART_RX_PIN,
#if defined(SL_IOSTREAM_EUSART_MCU_UART_CTS_PORT)
    .cts_port = SL_IOSTREAM_EUSART_MCU_UART_CTS_PORT,
    .cts_pin = SL_IOSTREAM_EUSART_MCU_UART_CTS_PIN,
#endif
#if defined(SL_IOSTREAM_EUSART_MCU_UART_RTS_PORT)
    .rts_port = SL_IOSTREAM_EUSART_MCU_UART_RTS_PORT,
    .rts_pin = SL_IOSTREAM_EUSART_MCU_UART_RTS_PIN,
#endif
  };

  sl_iostream_dma_config_t rx_dma_config_mcu_uart = {.src = (uint8_t *)&SL_IOSTREAM_EUSART_MCU_UART_PERIPHERAL->RXDATA,
                                                        .xfer_cfg = IOSTREAM_LDMA_TFER_CFG_PERIPH(SL_IOSTREAM_EUSART_RX_DMA_SIGNAL(SL_IOSTREAM_EUSART_MCU_UART_PERIPHERAL_NO))};

  sl_iostream_dma_config_t tx_dma_config_mcu_uart = {.dst = (uint8_t *)&SL_IOSTREAM_EUSART_MCU_UART_PERIPHERAL->TXDATA,
                                                        .xfer_cfg = IOSTREAM_LDMA_TFER_CFG_PERIPH(SL_IOSTREAM_EUSART_TX_DMA_SIGNAL(SL_IOSTREAM_EUSART_MCU_UART_PERIPHERAL_NO))};

  sl_iostream_uart_config_t uart_config_mcu_uart = {
    .rx_dma_cfg = rx_dma_config_mcu_uart,
    .tx_dma_cfg = tx_dma_config_mcu_uart,
    .rx_buffer = rx_buffer_mcu_uart,
    .rx_buffer_length = SL_IOSTREAM_EUSART_MCU_UART_RX_BUFFER_SIZE,
    .enable_high_frequency = _SL_IOSTREAM_EUSART_MCU_UART_ENABLE_HIGH_FREQUENCY, 
    .lf_to_crlf = SL_IOSTREAM_EUSART_MCU_UART_CONVERT_BY_DEFAULT_LF_TO_CRLF,
    .rx_when_sleeping = SL_IOSTREAM_EUSART_MCU_UART_RESTRICT_ENERGY_MODE_TO_ALLOW_RECEPTION,
    .uart_periph = &uart_periph_mcu_uart
  };
  uart_config_mcu_uart.sw_flow_control = SL_IOSTREAM_EUSART_MCU_UART_FLOW_CONTROL_TYPE == SL_IOSTREAM_EUSART_UART_FLOW_CTRL_SOFT;
#if defined(SL_IOSTREAM_EUSART_MCU_UART_ASYNC_TX)
  uart_config_mcu_uart.async_tx_enabled = SL_IOSTREAM_EUSART_MCU_UART_ASYNC_TX;
#else
  uart_config_mcu_uart.async_tx_enabled = false;
#endif
  // Instantiate eusart instance 
  status = sl_iostream_eusart_init(&sl_iostream_mcu_uart,
                                  &uart_config_mcu_uart,
                                  &config_mcu_uart,
                                  &context_mcu_uart);
  ...
  if (!uart_config_mcu_uart.sw_flow_control) {
    switch (SL_IOSTREAM_EUSART_MCU_UART_FLOW_CONTROL_TYPE)
    {
  ...
  sl_cos_config_vcom((uint32_t) SL_IOSTREAM_EUSART_MCU_UART_BAUDRATE, flow_control);
...
void sl_iostream_eusart_init_instances(void)
{
  ...
  sl_iostream_eusart_init_mcu_uart();
...
void SL_IOSTREAM_EUSART_TX_IRQ_HANDLER(SL_IOSTREAM_EUSART_MCU_UART_PERIPHERAL_NO)(void)
{
  sl_iostream_eusart_irq_handler(&sl_iostream_mcu_uart);
}

void SL_IOSTREAM_EUSART_RX_IRQ_HANDLER(SL_IOSTREAM_EUSART_MCU_UART_PERIPHERAL_NO)(void)
{
  sl_iostream_eusart_irq_handler(&sl_iostream_mcu_uart);
}
                           ...
                           sl_power_manager_em_t to)
{
  ...
  if (to == SL_POWER_MANAGER_EM0) {
    ...
    if (sl_iostream_uart_mcu_uart_handle->stream.context != NULL) {
      sl_iostream_uart_wakeup(sl_iostream_uart_mcu_uart_handle);
    }
    ...
    if (sl_iostream_uart_mcu_uart_handle->stream.context != NULL) {
      sl_iostream_uart_prepare_for_sleep(sl_iostream_uart_mcu_uart_handle);
    }
